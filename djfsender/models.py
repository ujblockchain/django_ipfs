...
class FileSender(models.Model):
    ...
    file_hash = models.CharField(
        max_length=200, help_text='sha256 hash of file before upload to pinata'
    )
    ipfs_hash = models.CharField(max_length=300)
    pin_size = models.IntegerField(help_text='size in kb', default=0)
    pin_time_stamp = models.CharField(
        max_length=100, help_text='time file was saved in pinata', default=''
    )
    file_description = models.TextField(help_text='file description')
    created = models.DateTimeField(
        auto_now_add=True, help_text='time record was created in model'
    )
    last_updated = models.DateTimeField(
        default=datetime.now(), help_text='time record was updated in model'
    )
